...
import { Share2, Trash2, Edit2, UserCheck, UserX, MessageCircle, AlertTriangle } from 'lucide-react';
...
const ManageQuestPage: React.FC<ManageQuestPageProps> = ({ questId, onBack, onEdit, onChatWithApplicant, onViewProfile }) => {
  ...
  function getQuestDisplayInfo(q: typeof quest, uid: string | null) {
    ...
    const isCreator = q.creatorId === uid;
    if (isCreator) {
      if (q.status === 'available') {
        if ((q.applicantIds?.length || 0) > 0) {
  ...
  const renderEvidenceSection = () => {
    if (quest.status !== 'pending' && quest.status !== 'disputed') return null;
    ...
    >
      <div className="pb-40">
        ...
        <QuestDetailContent
          ...
          extraSection={renderApplicantsSection()}
        />
      </div>
    </PageLayout>
  );
